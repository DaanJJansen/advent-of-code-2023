...
class Hand():
    ...
    def calculate_score(self):
        first_level_score = Scores.calculate_score(self.calculate_counts())
        second_level_score = "".join([card.rank for card in self.cards])
        return str(first_level_score) + second_level_score
    
    def calculate_counts(self):
        jokers = [card for card in self.cards if card.symbol == "J"]
        other_cards = [card for card in self.cards if card.symbol != "J"]

        counts = list(set((card,other_cards.count(card)) for card in other_cards))

        if not len(counts):
            return [[Card.get("J"), len(jokers)]]
        
        counts.sort(key=lambda x: x[1], reverse=True)
        lst_counts = [list(count) for count in counts]
        lst_counts[0][1] = lst_counts[0][1] + len(jokers)
        return lst_counts
...
class Scores():
    ...
    @classmethod
    def calculate_score(cls, counts: List):
